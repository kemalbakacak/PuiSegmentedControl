...
@objc public protocol PuiSegmentedControlDelegate: NSObjectProtocol {
...
open class PuiSegmentedControl: UIControl {
	...
	private func changeSeperatorVisibility(index: Int) {
		...
		if index == 0 {
			// If count of all sections equals to 2, don't hide.
	...
	public func scrollSegmentedControl(ratio: CGFloat,
									   nextViewSelectionThreshold: CGFloat,
									   currentIndex: Int,
									   destinationIndex: Int) {
		// For Layoutsubviews control
		self.isDraggingSelectedView = true
		
		let isMovingToRight = (destinationIndex > currentIndex)
		
		// Get current view
		let currentView = self.selectedViews[currentIndex]
		
		// If ratio more than percent threshold, then update next view
		if ratio > nextViewSelectionThreshold {
			self.configureViewAfterTransition(oldIndex: currentIndex, newIndex: destinationIndex)
		}
		...
		if self.isEqualWidth {
			...
			if isMovingToRight {
				self.selectedView.frame.origin.x = currentView.position + (currentView.width * ratio)
			} else {
				self.selectedView.frame.origin.x = currentView.position - (currentView.width * ratio)
			}
		} else {
			// Get destination view
			let destinationView = self.selectedViews[destinationIndex]
	...
	public func scrollViewDidEndDecelerating(at index: Int) {
